...
- Каждые **15 минут** (`CHECK_INTERVAL_MINUTES`) запрашивает из PostgreSQL все проекты с непустым `feed_url` (`Database.get_all_projects`). До этого планировщик читал `feed_url` из Redis-хеша `project:{id}`, в который это поле никогда не писалось (тот хеш используется только для `products_count`, см. [database.md](database.md)) — из-за этого автообновление молча не срабатывало вообще ни для одного проекта; исправлено.
- Для каждого проекта: если фид не обновлялся дольше **4 часов** (`UPDATE_INTERVAL_HOURS`, статус читается из `project:{id}:feed` в Redis, который реально пишется при загрузке) — запускает тот же пайплайн, что и ручная загрузка (шаги 2–6 выше), включая CPU-тяжёлую индексацию (без выноса в поток — здесь эта проблема пока не исправлена). Поля `auto_update` (включить/выключить автообновление по проекту) в БД не существует — автообновление сейчас не отключается per-project.

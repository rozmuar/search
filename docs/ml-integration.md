...
> **Статус: roadmap, не подключено.** Код в `src/ml/` (`embeddings.py`, `hybrid_search.py`, `reranker.py`, `spell_checker.py`, `vector_store.py`, ~2000 строк) существует и в целом соответствует описанному ниже дизайну, но `src/api/main.py` его нигде не импортирует и не вызывает — сейчас работает только классический поиск из [search-algorithm.md](search-algorithm.md). Всё, что ниже, — план, а не текущее поведение.
